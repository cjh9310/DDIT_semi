   	...
   	<section class="content">
   		<div class="card">
   			<div class="card-header with-border">
   				<button type="button" class="btn btn-primary" style="visibility:${member.authority eq 0 ? 'visible' : 'hidden'};"  onclick="OpenWindow('registForm.do','글쓰기',1000,700);" >글쓰기</button>
		    			...
		    			<tr>
		     			...
		     			<c:if test="${!empty newsList }" >
		            		<c:forEach items="${newsList }" var="news">		     						     				
		     					 <tr  onclick="OpenWindow('detail.do?newsno=${news.newsno}','글내용',700,800);" style="cursor:pointer;">
		            		  	   	<td>${news.newsno }</td>
				              		<td>${news.title }</td>
				              		<td>관리자</td>
		            		  	   	<td><fmt:formatDate value="${news.regdate }" pattern="yyyy-MM-dd"/></td>
		            		  	  </tr>	
